...
Generated by: AI Marketing Agents
  ...
  // Generate iCal file directly in frontend
  const generateICalFile = (launchData: any) => {
...
END:VEVENT`;
    ...
    if (launchData.timeline) {
      launchData.timeline.forEach((phase: any, index: number) => {
        if (phase.activities && phase.activities.length > 0) {
          phase.activities.forEach((activity: string, actIndex: number) => {
...
END:VCALENDAR`;
  ...
  // Productivity Integration Functions
  const exportToCalendar = async (calendarType: 'google' | 'ical') => {
    if (!result) return

    try {
      if (calendarType === 'ical') {
        // Generate and download iCal file directly
        const icalContent = generateICalFile(result);
        const blob = new Blob([icalContent], { type: 'text/calendar' });
        const url = window.URL.createObjectURL(blob);
        const a = document.createElement('a');
        a.href = url;
        a.download = `product-launch-${result.productName || 'calendar'}.ics`;
        document.body.appendChild(a);
        a.click();
        window.URL.revokeObjectURL(url);
        document.body.removeChild(a);
        
        alert('✅ iCal file downloaded successfully!');
      } else {
        // Google Calendar integration
        const response = await makeAuthenticatedRequest('/api/ai-tools/productivity/calendar', {
          method: 'POST',
          body: JSON.stringify({
            launchData: result,
            calendarType: 'google'
  ...
  // Generate Notion template directly in frontend
  const generateNotionTemplate = (launchData: any) => {
    const timelineBlocks = launchData.timeline?.map((phase: any) => 
      `**${phase.phase}** (${phase.timeline})\n` +
    ...
    return `# 🚀 Product Launch: ${launchData.productName || 'Product'}
...
*Copy this content into your Notion database manually*`;
  ...
  const exportToNotion = async () => {
    ...
    try {
      // Generate and download Notion template directly
      const notionContent = generateNotionTemplate(result);
      const blob = new Blob([notionContent], { type: 'text/markdown' });
      const url = window.URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = `notion-template-${result.productName || 'product'}.md`;
      document.body.appendChild(a);
      a.click();
      window.URL.revokeObjectURL(url);
      document.body.removeChild(a);
      
      alert('✅ Notion template downloaded! Copy this content into your Notion database.');
    } catch (error) {
  ...
  // Generate CSV data directly in frontend
  const generateCSVData = (launchData: any) => {
    const csvData = {
      'Launch Overview': [
      ...
      'Timeline': [
        ...
        (launchData.timeline || []).map((phase: any) => [
          phase.phase,
          phase.timeline,
      ...
      'Content Calendar': [
        ...
        (launchData.contentCalendar || []).flatMap((week: any) => 
  ...
  const exportToGoogleSheets = async () => {
    ...
    try {
      // Generate and download CSV file directly
      const csvContent = generateCSVData(result);
      const blob = new Blob([csvContent], { type: 'text/csv' });
      const url = window.URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = `product-launch-${result.productName || 'data'}.csv`;
      document.body.appendChild(a);
      a.click();
      window.URL.revokeObjectURL(url);
      document.body.removeChild(a);
      
      alert('✅ CSV file downloaded successfully! Import this into Google Sheets.');
    } catch (error) {
  ...
  // Generate ClickUp template directly in frontend
  const generateClickUpTemplate = (launchData: any) => {
    const subtasks = launchData.timeline?.map((phase: any) => 
      phase.activities?.map((activity: string) => 
        `- **Name:** 📋 ${activity}\n  **Description:** Phase: ${phase.phase}\n  **Status:** To Do\n  **Priority:** Medium`
      ).join('\n')
    ).join('\n') || 'N/A';

    return `# ClickUp Import Template
...
*Copy these tasks into ClickUp manually*`;
  ...
  const exportToClickUp = async () => {
    ...
    try {
      // Generate and download ClickUp template directly
      const clickupContent = generateClickUpTemplate(result);
      const blob = new Blob([clickupContent], { type: 'text/markdown' });
      const url = window.URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = `clickup-template-${result.productName || 'product'}.md`;
      document.body.appendChild(a);
      a.click();
      window.URL.revokeObjectURL(url);
      document.body.removeChild(a);
      
      alert('✅ ClickUp template downloaded! Copy these tasks into ClickUp manually.');
    } catch (error) {
      console.error('ClickUp export error:', error);
      alert('Failed to export to ClickUp. Please try again.');
    }
  }
              ...
              <div className="space-y-6">
                ...
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>
                      Export your launch plan to your favorite productivity tools
                    </CardDescription>
                  </CardHeader>
                  <CardContent className="space-y-4">
                    ...
                    <div className="flex items-center justify-between p-3 border rounded-lg">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-gray-600">Create calendar events for your launch timeline</p>
                        </div>
                      </div>
                    ...
                    <div className="flex items-center justify-between p-3 border rounded-lg">
                      <div className="flex items-center gap-3">
                        <div className="w-5 h-5 bg-black text-white rounded flex items-center justify-center text-xs font-bold">N</div>
                        <div>
                          <p className="font-medium">Notion Export</p>
                          <p className="text-sm text-gray-600">Export to Notion database with structured content</p>
                        </div>
                      </div>
                      <Button
                        size="sm"
                    ...
                    <div className="flex items-center justify-between p-3 border rounded-lg">
                      <div className="flex items-center gap-3">
                        <div className="w-5 h-5 bg-green-500 text-white rounded flex items-center justify-center text-xs font-bold">G</div>
                        <div>
                          <p className="font-medium">Google Sheets</p>
                          <p className="text-sm text-gray-600">Export structured data for spreadsheet analysis</p>
                        </div>
                      </div>
                      <Button
                        size="sm"
                    ...
                    <div className="flex items-center justify-between p-3 border rounded-lg">
                      <div className="flex items-center gap-3">
                        <div className="w-5 h-5 bg-blue-500 text-white rounded flex items-center justify-center text-xs font-bold">C</div>
                        <div>
                          <p className="font-medium">ClickUp Tasks</p>
                          <p className="text-sm text-gray-600">Create project tasks and subtasks</p>
                        </div>
                      </div>
                      <Button
                        size="sm"
